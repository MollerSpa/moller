...
class MeliActionSynchro(models.TransientModel):
    ...
    def get_data_id(self,id_vex,server):
        res = super(MeliActionSynchro, self).get_data_id(id_vex,server)
        if self.server_vex.conector == 'meli':
    ...
    def meli_api(self, server, query, accion,filtro):
        ...
        if query == "products":
            ...
            #raise ValidationError(lines_wait)
            if not lines_wait:
                self.env['vexlines.import'].search([('accion', '=', accion.id),
                                                                 ('instance', '=', server.id),
                                                                 ('instance', '=', server.id)]).unlink()
            ...
            if lines_wait:
                ...
                res = res[filtro]
            for r in res:
                item = self.get_data_id(r, server)
            ...
            # raise ValidationError(products_url)
            #raise ValidationError(len(array_products))
    ...
    def import_all(self,server,accion):
        ...
        res = super(MeliActionSynchro, self).import_all(server,query)

        if server.conector == 'meli':
            #raise ValidationError('whatt')
            data_request = self.meli_api(server, query,accion ,'results')
            self.synchro_threading(data_request,query, server, table, accion, None , api)
        return res
    ...
    def synchro_ext(self,dr, query, server, table, accion, id_vex , api ,exist):
        res = super(MeliActionSynchro, self).synchro_ext(dr, query, server, table, accion, id_vex , api )
        if server.conector == 'meli':
            if query == "products":
