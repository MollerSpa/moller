...
class WooSynchro(models.TransientModel):
    ...
    def import_all(self, server, query):
        return 0
    ...
    def synchro_unit_wizard(self):
        self.synchro_unit(self.accion, self.server_vex, self.id_vex)

    def synchro_unit(self, accion, model , server_vex, id_vex, api=None):
        item = self.get_data_id(id_vex, server_vex)
        #raise ValidationError(str(item))
        self.synchro(item, 'products', server_vex, model, accion, id_vex, None)

    def json_execute_create(self,table,data):
        _logger.info("Create new Line")
        _logger.info(data)
        #if table == 'product.product' and 'product_id' in dict.keys() and data['product_id'] == False: return
    ...
    def json_fields(self,data,query,server):
        return 0
    def synchro_ext(self, data, query, server, table, accion,id_vex ,api):
        return 0
    ...
    def start_import(self):
        return 0
    def vex_import(self, server, id_action, wcapi):
    ...
    def insert_lines(self,lines,server,creado , accion):
        ...
        for p in lines:
            ...
            if not existe:
                ...
                '''
                accion.log += self.env['vex.logs'].new({
